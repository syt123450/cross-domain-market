...
function getTop5DataOfStore($top5KeyWord, $storeID) {
    ...
    if ($top5KeyWord == "highestRated"){
        // Rated times from high to low
        $top5Products = getData("db272.TopProduct", ['storeID' => (int)$storeID], ['sort' => ['rated' => -1], 'limit' => 5]);
    }
    else if ($top5KeyWord == "bestPriced"){
        // Rates value from high to low
        $top5Products = getData("db272.TopProduct", ['storeID' => (int)$storeID], ['sort' => ['rate_price' => -1], 'limit' => 5]);
    }
    else if ($top5KeyWord == "mostLiked"){
        // Rates value from high to low
        $top5Products = getData("db272.TopProduct", ['storeID' => (int)$storeID], ['sort' => ['rate_like' => -1], 'limit' => 5]);
    }
    else if ($top5KeyWord == "bestQuality"){
        // Rates value from high to low
        $top5Products = getData("db272.TopProduct", ['storeID' => (int)$storeID], ['sort' => ['rate_quality' => -1], 'limit' => 5]);
    }
    else if ($top5KeyWord == "highestPrice"){
        // Price from high to low
        $top5Products = getData("db272.TopProduct", ['storeID' => (int)$storeID], ['sort' => ['priceNew' => -1], 'limit' => 5]);
    }
    else if ($top5KeyWord == "lowestPrice"){
        // Price from low to high
        $top5Products = getData("db272.TopProduct", ['storeID' => (int)$storeID], ['sort' => ['priceNew' => 1], 'limit' => 5]);
        ...
        // Default to return most viewed product
        $top5Products = getData("db272.TopProduct", ['storeID' => (int)$storeID], ['sort' => ['viewed' => -1], 'limit' => 5]);
    }
